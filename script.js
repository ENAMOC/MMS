...
function handleMultiSelect(e) {
    ...
    const dropdown = document.createElement('div'); dropdown.style.cssText = 'position:fixed;background:white;border:1px solid #ccc;border-radius:16px;box-shadow:0 6px 20px rgba(0,0,0,0.2);min-width:240px;z-index:1000;padding:12px;';
    ...
    const rect = e.target.getBoundingClientRect(); dropdown.style.top = `${rect.bottom + window.scrollY + 4}px`; dropdown.style.left = `${rect.left + window.scrollX - 60}px`;
    const listDiv = dropdown.querySelector('#dropdownList'), searchInput = dropdown.querySelector('#dropdownSearch');
    function renderList(filter = '') { const filtered = sorted.filter(v => filter === '' || v.toLowerCase().includes(filter.toLowerCase())); listDiv.innerHTML = ''; filtered.forEach(val => { const isChecked = curSet.has(val); const itemDiv = document.createElement('div'); itemDiv.style.display = 'flex'; itemDiv.style.alignItems = 'center'; itemDiv.style.gap = '8px'; itemDiv.style.padding = '5px 0'; itemDiv.innerHTML = `<input type="checkbox" value="${escapeHtml(val)}" ${isChecked ? 'checked' : ''}> <span style="font-size:12px;">${escapeHtml(val.length > 50 ? val.slice(0,47)+'...' : val)}</span>`; listDiv.appendChild(itemDiv); }); }
    renderList(); searchInput.addEventListener('input', (e) => renderList(e.target.value));
    dropdown.querySelector('#dropdownOk').addEventListener('click', () => { const newSet = new Set(); dropdown.querySelectorAll('#dropdownList input:checked').forEach(cb => newSet.add(cb.value)); if(newSet.size) columnSelectedValues[col] = newSet; else delete columnSelectedValues[col]; persistAndRefreshUI(); rebuildFilterCards(); dropdown.remove(); activeDropdown = null; });
    dropdown.querySelector('#dropdownClear').addEventListener('click', () => { delete columnSelectedValues[col]; persistAndRefreshUI(); rebuildFilterCards(); dropdown.remove(); activeDropdown = null; });
